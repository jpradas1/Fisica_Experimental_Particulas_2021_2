...
std::string histname = "hist_mzp";
float lumi = 1000.;
//int rebin = 20; //20 - ZmInv //4 - wmt //1 - event selection
bool logy = true;
std::string signalmass = "1750";
...
void HistPlotter(){
  ...
  leg->SetHeader(" #sqrt{s} = 8 TeV, 1 fb^{-1}, Camilo");
  ...
  if(logy) can->SaveAs(("./../../../../Graphs/"+outfile+signalmass+"_log.pdf").c_str());
  else can->SaveAs(("./../../../../Graphs/"+outfile+signalmass+".pdf").c_str());
}
